...
// HashPassword hashed password from user input
func HashPassword(password string) (string, error) {
	bytes, err := bcrypt.GenerateFromPassword([]byte(password), 14)
	return string(bytes), err
}

// CheckPasswordHash check password hash and password from user input if the match
...
// Prepare strings user input of any white spaces
func (u *User) Prepare() {
...
func (u *User) Validate(action string) error {
	...
	case "login":
		...
		return nil
	default: // this is for creating a use, where all fields are required
		if u.FirstName == "" {
